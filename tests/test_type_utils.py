...
from tha_utils_helper import ThaType
...
def test_normalize_bool_false_bool() -> None:
    assert ThaType.normalize_bool(False) is False


def test_normalize_bool_int_one() -> None:
    assert ThaType.normalize_bool(1) is True


def test_normalize_bool_int_zero() -> None:
    assert ThaType.normalize_bool(0) is False


def test_normalize_bool_string_true_variants() -> None:
    for val in ("true", "True", "TRUE", "yes", "Yes", "1", "t", "y"):
        assert ThaType.normalize_bool(val) is True, f"expected True for {val!r}"


def test_normalize_bool_string_false_variants() -> None:
    for val in ("false", "False", "FALSE", "no", "No", "0", "f", "n"):
        assert ThaType.normalize_bool(val) is False, f"expected False for {val!r}"
...
def test_normalize_bool_invalid_raises() -> None:
    with pytest.raises(ValueError):
        ThaType.normalize_bool("maybe")


def test_normalize_bool_none_raises() -> None:
    with pytest.raises((ValueError, TypeError)):
        ThaType.normalize_bool(None)
...
def test_safe_int_valid_int() -> None:
    assert ThaType.safe_int(7) == 7


def test_safe_int_float_string() -> None:
    assert ThaType.safe_int("3.14") is None


def test_safe_int_invalid_string() -> None:
    assert ThaType.safe_int("abc") is None


def test_safe_int_none() -> None:
    assert ThaType.safe_int(None) is None


def test_safe_int_empty_string() -> None:
    assert ThaType.safe_int("") is None
...
def test_safe_float_int_string() -> None:
    assert ThaType.safe_float("42") == 42.0


def test_safe_float_valid_float() -> None:
    assert ThaType.safe_float(1.5) == 1.5


def test_safe_float_invalid_string() -> None:
    assert ThaType.safe_float("abc") is None


def test_safe_float_none() -> None:
    assert ThaType.safe_float(None) is None


def test_safe_float_empty_string() -> None:
    assert ThaType.safe_float("") is None
...
def test_normalize_bool_rows_out_column() -> None:
    rows = [{"active": "yes"}]
    result = ThaType.normalize_bool_rows(rows, "active", out_column="active_bool")
...
def test_safe_int_rows_out_column() -> None:
    rows = [{"count": "42"}]
    result = ThaType.safe_int_rows(rows, "count", out_column="count_int")
...
def test_safe_float_rows_out_column() -> None:
    rows = [{"amount": "3.14"}]
    result = ThaType.safe_float_rows(rows, "amount", out_column="amount_float")
